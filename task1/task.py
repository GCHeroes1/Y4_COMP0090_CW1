...
def fit_polynomial_sgd(x_coordinates, t_target_values, degree, learning_rate, minibatch_size):
    ...
    #     vander = create_features(x_coordinates, degree=degree, standardize=False)
    #     dataset.append([vander, t_target_values[i]])

    features = make_features(x_coordinates, degree=degree)
    dataset = TensorDataset(features, t_target_values)

    training_loader = DataLoader(dataset, batch_size=3, shuffle=True, num_workers=1)
    ...
    for epoch in range(epochs):
        ...
        for i, data in enumerate(training_loader, 0):
            ...
            if i % 20 == 19:  # print every 5 mini-batches
